<template>
    <div>
        <q-table :data="data" :columns="columns" row-key="fch_mes" dense :pagination="pagination" separator="vertical">
            <template v-slot:top>
                <div v-if="symbol_value != ''">
                    <span class="text-blue-10 text-h6 q-pr-xs">{{ symbol_value }}</span><span class="text-body1">{{
                        symbol_nombre }}</span>
                </div>
                <div v-if="symbol_value == ''">
            ...
            <template v-slot:body="props">
                <q-tr :props="props" :class="props.row.num_dia_semana == 1 ? 'text-green text-bold' : 'text-black'">
                    <q-menu touch-position context-menu>
                        <q-list dense style="min-width: 100px">
                            <q-item class="bg-blue-10 text-white text-body1" clickable v-close-popup
                                @click="abrir_criterios_calculos_variacion_diaria(props.row)">
                                <q-item-section>
                                    <div class="q-pl-sm">Calcular variacion desde antiguedad</div>
                                </q-item-section>
                            </q-item>
                        </q-list>
                    </q-menu>
                    ...
                    <q-td key="imp_variacion_cierre"
                        v-bind:class="{ 'bg-green': props.row.imp_variacion_cierre >= 0, 'bg-red': props.row.imp_variacion_cierre < 0, 'text-right': true, 'text-white': true }">
                        {{ props.row.imp_variacion_cierre }}
                    </q-td>
                    <q-td key="imp_variacion_maximo"
                        :class="{ 'bg-green': props.row.imp_variacion_maximo >= 0, 'bg-red': props.row.imp_variacion_maximo < 0, 'text-right': true, 'text-white': true }">
                        {{ props.row.imp_variacion_maximo }}
                    </q-td>
                    <q-td key="imp_variacion_minimo"
                        :class="{ 'bg-red': props.row.imp_variacion_minimo <= -1, 'bg-orange': props.row.imp_variacion_minimo > -1, 'text-white': true, 'text-right': true }">
                ...
                <q-tr v-if="props.row.num_dia_semana == 1">
        ...
        <WinSimulacionVariacionAntiguedad v-model="WinSimulacionVariacionAntiguedad.open"
            ...
            :fch_final="WinSimulacionVariacionAntiguedad.fch_final" v-on:btn-aceptar-click="simular" />
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        copiarAlPortapapeles: function () {
            ...
            const header = [
                "Año", "Fecha", "Imp. Cierre anterior", "Imp. Apertura", "Imp. Maximo",
                "Imp. Minimo", "Imp. Cierre", "Var. Cierre", "Var. Maximo", "Var. Minimo",
                "Pct. Cierre", "Pct. Maximo", "Pct. Minimo"
            ].join("\t");
            const rows = this.data.map(row => {
                return [
                    row.anyo, row.fch_serie, row.imp_cierre_ant, row.imp_apertura,
