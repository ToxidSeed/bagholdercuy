<template>
    <div>
        <q-table :data="metricas.state.table_metricas_var_negativa.data" :columns="columns" row-key="name" dense
            separator="vertical" :pagination="pagination" flat hide-bottom>
            <template v-slot:top>
                <div class="row q-gutter-md">
                    <div class="text-red-10 text-subtitle1">Cierre negativos</div>
                    <q-separator vertical />
                    <div>{{ metricas.state.table_metricas_var_negativa.count }}/{{
                        metricas.state.table_metricas_var_negativa.total }}</div>
                </div>
            </template>
...
/*
call 797.5 - 1.05 05/jan
sell put 475 - 482

*/

export default {
    name: "TableMetricasVariacionNegativa",
    data() {
        return {
            data: [],
            columns: [{
