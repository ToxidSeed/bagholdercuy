            ...
            <q-card-section class="q-pt-none">
                ...
                <div class="row justify-between">
                    <div class="q-pt-md text-subtitle2 text-blue-10">Reorganizar fecha: 
                        <span class="text-h6 text-black">{{filter.fch_hr_transaccion}}</span>                                                
                    </div>                    
                    <div>
...
export default {
    ...
    data: () => {
        return {
            filter:{
                fch_hr_transaccion:"No seleccionada"
            },            
            visible:{
    ...
    methods:{
        ...
        select:function(row){                        
            this.filter = {
                fch_hr_transaccion:row.fch_hr_transaccion
            }
            console.log('cambiar-fecha')
            this.$emit('cambiar-fecha',row.fch_hr_transaccion)   
        },
        procesar:function(){
            ...
            this.$http.post(
                '/FundsManager/ReorganizarController/procesar',{
                    fch_reorganizar: this.filter.fch_hr_transaccion,
                ...
                this.$refs.TableTransaccionesFondosFecha.get_transacciones_x_fecha(this.filter.fch_hr_transaccion)
